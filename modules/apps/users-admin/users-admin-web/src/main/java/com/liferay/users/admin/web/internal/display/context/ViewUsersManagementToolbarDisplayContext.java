...
public class ViewUsersManagementToolbarDisplayContext {
	...
	public List<DropdownItem> getFilterDropdownItems() {
		return DropdownItemListBuilder.addGroup(
			...
			dropdownGroupItem -> {
				...
				dropdownGroupItem.setLabel(
					LanguageUtil.get(
						_httpServletRequest, "filter-by-domain"));
			}
		).addGroup(
			dropdownGroupItem -> {
	...
	public SearchContainer getSearchContainer() {
		...
		if (_domain.equals("company-users")) {
			params.put("accountEntryIds", new Long[]{});
		}
		else if (_domain.equals("account-users")) {
			params.put("accountEntryIds", new Long[]{AccountConstants.ACCOUNT_ENTRY_ID_ANY});
		}
	...
	private List<DropdownItem> _getFilterDomainDropdownItems() {
		...
		for (String domain : new String[] {"all", "company-users", "account-users"}) {
			domainDropdownitems.add(
				dropdownItem -> {
					dropdownItem.setActive(domain.equals(_domain));
					dropdownItem.setHref(
						getPortletURL(), "domain", domain);
					dropdownItem.setLabel(
